...
import { useSiteMaintenance, type SiteMaintenanceFormData } from '@/hooks/orcamentos/useSiteMaintenance';
...
export default function Canteiro() {
  ...
  const { items, total, isLoading, createItem, updateItem, deleteItem } = useSiteMaintenance(selectedRevision?.id);

  const [newItem, setNewItem] = useState<SiteMaintenanceFormData>({
    descricao: '',
    valor_mensal: 0,
    meses: 1,
  });
  ...
  return (
    <Card>
      <CardHeader className="pb-3">
        <CardTitle className="text-lg">Manutenção de Canteiro</CardTitle>
        <CardDescription>Custos recorrentes mensais do canteiro de obras</CardDescription>
      </CardHeader>
      <CardContent>
        <Table>
          <TableHeader>
          ...
          <TableBody>
            ...
            {!lockState.isLocked && (
              <TableRow className="bg-muted/30">
                <TableCell>
                  <Input
                    placeholder="Ex: Aluguel container, Energia elétrica..."
      ...
      <CardFooter className="border-t pt-4">
        <div className="flex justify-end w-full">
          <div className="text-right">
            <p className="text-sm text-muted-foreground">Total Canteiro</p>
